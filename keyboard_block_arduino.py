...
class KeyboardBlockArduino:
    ...
    def __init__(
        self, block_simulate_input=True, ignore_window_ms=200, enable_logging=False
    ):
        self.hook = None
        self.running = False
        ...
        # Stats
        self.stats = {"total_events": 0, "arduino_blocked": 0, "hardware_allowed": 0}
        ...
        self.CONFIG_PATH = os.path.join(
            os.path.dirname(__file__), "keyboard_block_arduino.config.json"
        )
        self._load_config()
        ...
        try:
            if os.path.exists(self.CONFIG_PATH):
                with open(self.CONFIG_PATH, "r", encoding="utf-8") as f:
                    config = json.load(f)
                    self.block_simulate_input = config.get(
                        "block_simulate_input", self.block_simulate_input
                    )
                    self.ignore_window_ms = config.get(
                    ...
                    if self.enable_logging:
                        print(
                            f"[CONFIG] Loaded: block_simulate_input={self.block_simulate_input}, "
    ...
    def install_hook(self):
        ...
        print(
            f"[HOOK] Keyboard hook installed (block_simulate_input={self.block_simulate_input})"
        )
