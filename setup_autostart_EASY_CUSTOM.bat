...
if not exist "multiplicity_jitter_DESYNC_EASY_CUSTOM.exe" (
    if not exist "SystemAudioService.exe" (
        ...
        echo Please run compile_EASY_CUSTOM.bat or compile_EASY_CUSTOM_obfuscate.bat first.
        pause
        exit /b 1
    )
    set "EXE_NAME=SystemAudioService.exe"
) else (
    set "EXE_NAME=multiplicity_jitter_DESYNC_EASY_CUSTOM.exe"
)
...
if %errorlevel% equ 0 (
    ...
    echo.
    echo To disable auto-start, run: remove_autostart_EASY_CUSTOM.bat
    echo.
) else (
    echo.
