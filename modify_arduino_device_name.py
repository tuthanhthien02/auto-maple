        ...
        if os.path.isdir(base_path):
            ...
            try:
                version_folders = [
                    ...
                    and re.match(r"^\d+\.\d+\.\d+$", f)
                ]
                if version_folders:
                    # Use latest version
                    latest_version = sorted(
                        version_folders, key=lambda x: [int(i) for i in x.split(".")]
                    )[-1]
...
def modify_boards_txt(boards_txt_path, change_vid_pid=False):
    ...
    try:
        ...
        if change_vid_pid:
            ...
            print(f"   Product Name: {NEW_PRODUCT_NAME}")
            print(f"   Manufacturer: {NEW_MANUFACTURER_NAME}")
            print("   VID/PID: Not changed (disabled)")
        ...
        for i, line in enumerate(lines):
            if "pro" in line.lower() and any(
                ...
                for keyword in [
                    ...
                    NEW_MANUFACTURER_NAME.lower(),
                    NEW_VID.lower() if change_vid_pid else "",
...
def verify_modification(boards_txt_path, change_vid_pid=False):
    ...
    try:
        ...
        pro_product_pattern = re.search(
            r"pro\.menu\.usb\.\d+\.product=" + re.escape(NEW_PRODUCT_NAME),
            content,
            re.IGNORECASE,
        )
        pro_manufacturer_pattern = re.search(
            r"pro\.menu\.usb\.\d+\.manufacturer=" + re.escape(NEW_MANUFACTURER_NAME),
            content,
            re.IGNORECASE,
        )

        result = (has_product and has_manufacturer) or (
            pro_product_pattern and pro_manufacturer_pattern
        )
        ...
        if change_vid_pid and NEW_VID and NEW_PID:
            ...
            pro_vid_pattern = re.search(
                r"pro\.vid\.\d+=" + re.escape(NEW_VID), content, re.IGNORECASE
            )
            pro_pid_pattern = re.search(
                r"pro\.pid\.\d+=" + re.escape(NEW_PID), content, re.IGNORECASE
            )
            result = result and (
                (has_vid and has_pid) or (pro_vid_pattern and pro_pid_pattern)
            )
            if not result:
                print("[DEBUG] VID/PID verification:")
        ...
        if not result:
            print("[DEBUG] Verification details:")
...
def main():
    ...
    if not boards_txt_path:
        ...
        print(
            "  - C:\\Users\\<YOUR_USER>\\AppData\\Local\\Arduino15\\packages\\arduino\\hardware\\avr\\<VERSION>\\boards.txt"
        )
        return False

    print("[✓] Found boards.txt:")
