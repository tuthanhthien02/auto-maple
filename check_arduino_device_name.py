...
def check_arduino_device_name():
    ...
    try:
        ...
        result = user32.GetRawInputDeviceList(
            None, ctypes.byref(device_count), ctypes.sizeof(wintypes.HANDLE * 2)
        )
        ...
        result = user32.GetRawInputDeviceList(
            device_buffer, ctypes.byref(device_count), ctypes.sizeof(RAWINPUTDEVICELIST)
        )
                ...
                if name_size.value > 0:
                    ...
                    if result > 0:
                        ...
                        keyboard_devices.append(
                            {"handle": device.hDevice, "name": device_name}
                        )
                        ...
                        if any(
                            ...
                            for keyword in ["arduino", "micro", "leonardo"]
                        ):
