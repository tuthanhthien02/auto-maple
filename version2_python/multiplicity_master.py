...
def broadcast_key(key, action="KEYDOWN"):
    ...
    if DEBUG_MODE:
        total = len(VM_PORTS)
        status = f"✅ {success_count}/{total}" if fail_count == 0 else f"⚠️ {success_count}/{total} (Failed: {fail_count})"
...
def on_key_press(key):
    ...
    try:
        ...
        if key_name in KEYS_TO_BROADCAST or key_name in ARROW_KEYS:
            ...
            threading.Thread(
                target=broadcast_key,
                args=(key_name, "KEYDOWN"),
                daemon=True
            ).start()
...
def show_status():
    ...
    print(f"Broadcasting: {Fore.GREEN if broadcasting_enabled else Fore.RED}{'ENABLED' if broadcasting_enabled else 'DISABLED'}")
...
def show_performance():
    ...
    if stats['total_sent'] > 0:
        success_rate = (stats['total_success'] / (stats['total_sent'] * len(VM_PORTS))) * 100
...
def main():
    ...
    for vm_name, port in VM_PORTS.items():
        print(f"  • {vm_name}: {VM_HOST}:{port}")
    print(f"\n{Fore.YELLOW}Hotkeys:")
    print(f"  Ctrl+Alt+T - Toggle Broadcasting")
    print(f"  Ctrl+Alt+S - Show Status")
    print(f"  Ctrl+Alt+P - Show Performance")
    print(f"  Ctrl+Alt+Q - Quit")
    print(f"{Fore.CYAN}{'='*60}\n")
