...
SENSITIVE_PROCESSES = {
    # Virtual Machines
    'vmware': [
        'vmware.exe', 'vmwaretray.exe', 'vmwareuser.exe',
        'vmware-vmx.exe', 'vmware-vmx-debug.exe',
        'vmware-vmx-stats.exe', 'vmwareauthd.exe',
        'vmwarehostopen.exe', 'vmware-usbarbitrator.exe'
    ],
    'virtualbox': [
        'virtualbox.exe', 'vboxsvc.exe', 'vboxmanage.exe',
        'vboxheadless.exe', 'vboxsds.exe', 'vboxtray.exe',
        'vboxservice.exe', 'vboxautostart.exe'
    ],
    'vbox': [
        'vboxsvc.exe', 'vboxmanage.exe', 'vboxheadless.exe',
        'vboxsds.exe', 'vboxtray.exe', 'vboxservice.exe'
    ],
    'ldplayer': [
        'ldplayer.exe', 'ldbox.exe', 'ldconsole.exe',
        'ldmultiplayer.exe', 'ldstore.exe'
    ],
    'nox': [
        'nox.exe', 'noxvm.exe', 'noxplayer.exe'
    ],
    'bluestacks': [
        'bluestacks.exe', 'hd-player.exe', 'bstkhelper.exe'
    ],
    'memu': [
    ...
    'keyboard_hooks': [
        'keyboard_to_arduino.py', 'keyboard_block_arduino.py',
        'host_sender.py', 'vmware_receiver.py',
        'python.exe', 'pythonw.exe'  # Python processes running keyboard hooks
    ],
    
    # Key Remapping / Input Simulation Tools
    'key_remapping': [
        'sharpkeys.exe', 'keytweak.exe', 'keymapper.exe',
        'keyremapper.exe', 'remapkey.exe',
        'keyboardlayouts.exe', 'keylayout.exe', 'scancode.exe',
        'keyboardmaestro.exe', 'karabiner.exe', 'uikeyboard.exe'
    ],
    
    # Virtual Keyboard / Input Simulation
    'virtual_keyboard': [
        'vkbd.exe', 'virtualkeyboard.exe', 'onscreen.exe',
        'osk.exe', 'screenkeyboard.exe',
        'virtualkb.exe', 'keyboardemulator.exe'
    ],
    ...
    'injection': [
        'injector.exe', 'dllinjector.exe', 'processhacker.exe',
        'processexplorer.exe', 'procmon.exe'
    ],
    
    # Remote Desktop / Control
    'remote': [
        'teamviewer.exe', 'anydesk.exe', 'splashtop.exe',
        'vnc.exe', 'ultravnc.exe', 'tvnserver.exe'
    ],
    
    # Other Suspicious
    'other': [
        'sandboxie.exe', 'sandboxiedcomlaunch.exe',
        'sandboxierpcss.exe', 'sandboxiedllhost.exe'
    ]
}
...
class NGSRegistryChecker:
    ...
    SUSPICIOUS_SERVICES = [
        'keyboard', 'keyhook', 'keyremap', 'keymap', 'scancode',
        'vkbd', 'virtualkb', 'inputsim', 'sendinput', 'keylogger'
    ]
                ...
                try:
                    ...
                    for keyword in self.SUSPICIOUS_SERVICES:
                        if keyword in service_lower:
                            try:
                                service_path = winreg.OpenKey(services_key, service_name)
                                try:
                                    image_path, _ = winreg.QueryValueEx(service_path, 'ImagePath')
                                    found.append({
                                        'type': 'service',
    ...
    def check_startup_programs(self) -> List[Dict]:
        ...
        startup_keywords = [
            'keyboard', 'keyhook', 'keyremap', 'keymap', 'scancode',
            'vkbd', 'virtualkb', 'inputsim', 'autohotkey'
        ]
    ...
    def print_registry_report(self):
        ...
        print("=" * 70)
        print()
        print(f"⚠️  Found {len(items)} registry entry(ies) related to keyboard simulation:")
        print()
...
class NGSProcessChecker:
    ...
    def scan_processes(self) -> Tuple[int, Dict[str, int]]:
        ...
        self.process_by_category = {category: [] for category in SENSITIVE_PROCESSES.keys()}
                    ...
                    for pattern in patterns:
                        ...
                        if pattern_lower in proc_name or pattern_lower in proc_exe:
                            ...
                            if 'python' in pattern_lower:
                                if any(keyword in proc_cmdline for keyword in [
                                    'keyboard_to_arduino', 'keyboard_block_arduino',
                                    'host_sender', 'vmware_receiver'
                                ]):
        ...
        # Count by category
        counts = {category: len(processes) for category, processes in self.process_by_category.items()}
    ...
    def print_report(self, show_details: bool = True):
        ...
        print(f"⚠️  Found {total} sensitive process(es) that might trigger NGS detection:")
        print()
    ...
    def kill_processes(self, category: str = None, confirm: bool = True) -> int:
        ...
        for proc_info in processes_to_kill:
            try:
                ...
                print(f"❌ Failed to kill PID {proc_info['pid']}: {proc_info['name']} - {e}")
            except Exception as e:
...
def main():
    ...
    parser.add_argument(
        ...
        help='Show brief report without details'
    )
    parser.add_argument(
