...
class RandomizationSmokeTests(unittest.TestCase):
    ...
    def test_iter_commands_skip_teleport_in_reverse(self):
        ...
        decisions = list(
            point._iter_commands(is_reverse_variant=True, is_floor_reverse=False)
        )
    ...
    def test_get_current_floor_position_legacy_and_custom(self):
        ...
        self.routine.labels = {
            label_floor.label: label_floor,
            label_mez.label: label_mez,
        }
    ...
    def test_load_floor_floor_json_metadata(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            meta_data = {
                "floor_descriptors": [
                    {"id": "roof", "labels": ["roof_label"], "priority": 3}
                ]
            }
            meta_path.write_text(json.dumps(meta_data), encoding="utf-8")
    ...
    def test_teleport_not_skipped_in_normal_variant(self):
        ...
        decisions = list(
            point._iter_commands(is_reverse_variant=False, is_floor_reverse=False)
        )
        self.assertEqual(len(decisions), 1)
        self.assertIsNone(decisions[0].skip_reason)
    ...
    def test_position_offset_applies_within_bounds(self):
        self.routine.position_offset_config.update(
            {"enabled": True, "range": 0.05, "axes": {"x": True, "y": True}}
        )
        original = (0.0, 1.0)
        ...
        self.assertGreater(
            self.routine.observability_metrics["offsets"], offsets_before
        )
    ...
    def test_bot_config_profile_switch(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sample_data = {
                ...
                "defaults": {"foo": {"bar": 1}},
                "profiles": {"default": {}, "custom": {"foo": {"bar": 5}}},
            }
    ...
    def test_get_feature_value_uses_bot_config_override(self):
        with patch.object(config, "bot_config", BotConfig()) as _:
            config.bot_config.set(
                "routine_randomization.point_selection.skip_probability", 0.33
            )
            from src.common.anti_detect_config import get_feature_value

            self.assertAlmostEqual(
                get_feature_value(
                    "routine_randomization.point_selection.skip_probability", 0.1
                ),
                0.33,
            )
    ...
    def test_routine_load_uses_bot_config_defaults(self):
        ...
        config.bot_config.set(
            "routine_randomization.movement.position_offset.enabled", True
        )
        config.bot_config.set(
            "routine_randomization.movement.position_offset.range", 0.02
        )
        config.bot_config.set(
            "routine_randomization.movement.observability.log_every_loops", 1
        )
    ...
    def test_observability_logging_snapshot(self):
        config.bot_config.set(
            "routine_randomization.movement.observability.log_every_loops", 3
        )
        self.routine._apply_config_fallbacks()
    ...
    def test_bot_config_set_persist(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path.write_text(
                json.dumps({"defaults": {}, "profiles": {"default": {}}}),
                encoding="utf-8",
            )
            bot_cfg = BotConfig(config_path)
    ...
    def test_get_feature_value_without_bot_config(self):
        ...
        with patch.object(config, "bot_config", None):
            expected = ANTI_DETECT_CONFIG["routine_randomization"]["point_selection"][
                "skip_probability"
            ]
            self.assertEqual(
                get_feature_value(
