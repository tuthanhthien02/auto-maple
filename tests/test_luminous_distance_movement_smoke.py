...
class LuminousDistanceMovementSmokeTests(unittest.TestCase):
    ...
    def test_horizontal_movement_far_distance_hold_key(self, bernoulli_mock, sleep_mock, 
                                                        press_mock, key_up_mock, key_down_mock):
        ...
        hold_threshold = settings.move_tolerance * luminous.TimingConfig.STEP_MOVEMENT['hold_threshold_multiplier']
    ...
    @patch('resources.command_books.luminous.time.sleep')
    @patch('resources.command_books.luminous.utils.bernoulli')
    def test_horizontal_movement_near_distance_press_key(self, bernoulli_mock, sleep_mock,
                                                          press_mock, key_up_mock, key_down_mock):
    ...
    def test_distance_calculation_when_not_provided(self, distance_mock, bernoulli_mock,
                                                      sleep_mock, press_mock, key_up_mock, key_down_mock):
    ...
    def test_log_far_distance_hold_key(self, log_mock, bernoulli_mock, sleep_mock,
                                       press_mock, key_up_mock, key_down_mock):
        ...
        target = (0.15, 0.0)
        distance = 0.15
        hold_threshold = settings.move_tolerance * 3
        
        luminous.step('right', target, distance=distance)
        
        # Verify log.debug was called with hold key message
        log_calls = [str(call) for call in log_mock.debug.call_args_list]
        assert any('distance xa' in str(call) and 'hold key' in str(call) for call in log_calls)
    ...
    @patch('resources.command_books.luminous.utils.bernoulli')
    @patch('resources.command_books.luminous.log')
    def test_log_near_distance_press_key(self, log_mock, bernoulli_mock, sleep_mock,
                                         press_mock, key_up_mock, key_down_mock):
        ...
        log_calls = [str(call) for call in log_mock.debug.call_args_list]
        assert any('distance gần' in str(call) and 'press key' in str(call) for call in log_calls)
    ...
    @patch('resources.command_books.luminous.time.sleep')
    @patch('resources.command_books.luminous.utils.bernoulli')
    def test_threshold_boundary_far(self, bernoulli_mock, sleep_mock,
                                    press_mock, key_up_mock, key_down_mock):
        """Test behavior at threshold boundary (slightly above = far)."""
        bernoulli_mock.return_value = False
        
        hold_threshold = settings.move_tolerance * 3  # 0.09
        # Distance slightly above threshold
        distance = hold_threshold + 0.001  # 0.091
    ...
    @patch('resources.command_books.luminous.time.sleep')
    @patch('resources.command_books.luminous.utils.bernoulli')
    def test_threshold_boundary_near(self, bernoulli_mock, sleep_mock,
                                     press_mock, key_up_mock, key_down_mock):
    ...
    @patch('resources.command_books.luminous.time.sleep')
    @patch('resources.command_books.luminous.utils.bernoulli')
    def test_both_directions_left_and_right(self, bernoulli_mock, sleep_mock,
                                            press_mock, key_up_mock, key_down_mock):
        ...
        left_calls = [call for call in key_down_mock.call_args_list if call[0][0] == 'left']
        assert len(left_calls) > 0
        
        # Verify right used press
        right_press_calls = [call for call in press_mock.call_args_list if call[0][0] == 'right']
