...
class DynamicPathsSmokeTests(unittest.TestCase):
    ...
    def test_generate_paths_creates_correct_count(self):
        ...
        self.assertGreaterEqual(len(self.routine.dynamic_paths), 1)  # At least path1 (full path)
        self.assertLessEqual(len(self.routine.dynamic_paths), self.routine.dynamic_paths_config['path_count'])
        
        # Path1 should always be full path
        path1 = next((p for p in self.routine.dynamic_paths if p['id'] == 'path1'), None)
    ...
    def test_generate_paths_all_paths_have_valid_indices(self):
        ...
        for path in self.routine.dynamic_paths:
            ...
            self.assertLessEqual(len(path['indices']), len(self.points))  # Not more than total
    ...
    def test_generate_paths_random_skip_strategy(self):
        ...
        with patch('src.routine.routine.random.uniform', return_value=0.2), \
             patch('src.routine.routine.random.sample', side_effect=lambda seq, k: sorted(seq)[:k]):
            self.routine._generate_dynamic_paths()
    ...
    def test_generate_paths_partial_strategy(self):
        ...
        with patch('src.routine.routine.random.uniform', return_value=0.2), \
             patch('src.routine.routine.random.random', return_value=0.0):  # First N%
            self.routine._generate_dynamic_paths()
    ...
    def test_select_initial_path(self):
        ...
        self.assertIn(self.routine.current_path_id, [p['id'] for p in self.routine.dynamic_paths])
        
        # Index should be set to first point in selected path
        current_path = next((p for p in self.routine.dynamic_paths if p['id'] == self.routine.current_path_id), None)
        if current_path:
    ...
    def test_select_next_path_random_mode(self):
        ...
        with patch('src.routine.routine.random.choice', side_effect=lambda seq: seq[1] if len(seq) > 1 else seq[0]):
    ...
    def test_apply_path_to_index(self):
        ...
        current_path = next((p for p in self.routine.dynamic_paths if p['id'] == 'path1'), None)
        if current_path:
            self.assertEqual(self.routine.index, current_path['indices'][0])
