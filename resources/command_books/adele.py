        ...
        while config.enabled and counter > 0 and error > settings.adjust_tolerance:
            if toggle:
                ...
                if abs(d_y) > settings.adjust_tolerance / math.sqrt(2):
                    if d_y < 0:
                        # Fallback if FlashJump is unavailable in this context
                        key_down("up")
                        key_up("up")
                    else:
                        key_down("down")
...
class Reign_Of_Destruction(Command):
    """
...
class Grave_Proclamation(Command):
