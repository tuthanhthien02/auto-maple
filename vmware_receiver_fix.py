...
class VMwareReceiver:
    ...
    def __init__(
        ...
        self.key_mapping = (
            key_mapping or {}
        )  # Key remapping dictionary: {'original': 'mapped'}
        ...
        self.CONFIG_PATH = os.path.join(
            os.path.dirname(__file__), "vmware_receiver.config.json"
        )
        self._load_config()
        ...
        try:
            if os.path.exists(self.CONFIG_PATH):
                with open(self.CONFIG_PATH, "r", encoding="utf-8") as f:
                    ...
                    self.block_local_input = config.get(
                    ...
                    if key_mapping_config:
                        ...
                        print(
                            f"[CONFIG] Key mappings: {self.key_mapping}"
                        )  # Debug: Print actual mappings
                        if self.enable_logging:
                            for orig, mapped in self.key_mapping.items():
                                print(f"[CONFIG]   {orig} → {mapped}")
                    else:
                        self.key_mapping = {}
                        print("[CONFIG] No key mapping found in config")
                    ...
                    if self.enable_logging:
                        print(
                            f"[CONFIG] Loaded: com_port={self.com_port}, "
    ...
    def is_serial_connected(self):
        """Kiểm tra xem serial đã kết nối và hoạt động chưa"""
        return self.serial and self.serial.is_open

    def connect_arduino(self) -> bool:
        """Kết nối với Arduino - nếu đã kết nối thì bỏ qua"""
        # Kiểm tra nếu đã kết nối rồi thì không kết nối lại
        if self.is_serial_connected():
            print(f"[ARDUINO] ✓ Already connected to {self.com_port}")
            return True
        ...
        try:
            ...
            time.sleep(0.5)
            print(
                f"[ARDUINO] ✓ Connected successfully to {self.com_port} (baudrate: {self.baudrate})"
            )
            # Sync state: release all keys
            self.send_all_up()
            print("[ARDUINO] Initialized - all keys released")
            return True
        except serial.SerialException as e:
            # Kiểm tra xem lỗi có phải do port đã được mở không
            if "Access is denied" in str(e):
                print(f"[ARDUINO] ⚠ Port {self.com_port} is already in use")
                print(
                    "[ARDUINO] ⚠ Assuming Arduino is already connected and continuing..."
                )

                # Đánh dấu là đã kết nối để tiếp tục sử dụng
                # Tạo một đối tượng serial giả để tránh lỗi None
                class DummySerial:
                    def __init__(self):
                        self.is_open = True
                ...
                print(
                    f"[ERROR] Check if Arduino is connected and {self.com_port} is correct"
                )
                return False
        except Exception as e:
    ...
    def send_key_to_arduino(self, key_name: str, action: str) -> bool:
        ...
        try:
            ...
            # Nếu là DummySerial (khi bị Access denied), chỉ ghi log
            if (
                hasattr(self.serial, "__class__")
                and self.serial.__class__.__name__ == "DummySerial"
            ):
                if self.enable_logging:
                    print(f"[FORWARD] {action}:{key_name} (dummy - port in use)")
            ...
            if self.enable_logging:
                print(
                    f"[FORWARD] {action}:{key_name} ({bytes_written} bytes) → Arduino"
                )
            return True
    ...
    def send_all_up(self):
        ...
        try:
            if self.is_serial_connected():
                # Nếu là DummySerial, chỉ ghi log
                if (
                    hasattr(self.serial, "__class__")
                    and self.serial.__class__.__name__ == "DummySerial"
                ):
                    if self.enable_logging:
                        print("[FORWARD] all_up (dummy - port in use)")
                    return
        ...
        if action in ("down", "up"):
            ...
            if self.remapping_enabled and key_name in self.key_mapping:
                ...
                if not self.remapping_enabled:
                    ...
                    if self.enable_logging:
                        print(
                            f"[PROCESS] Parsed: action='{action}', key='{key_name}' (no mapping for this key)"
                        )
    ...
    def start_server(self):
        ...
        try:
            ...
            print("[SERVER] Listening for Host sender connection...")
            print(
                f"[STATUS] Arduino: {'CONNECTED' if self.is_serial_connected() else 'DISCONNECTED'}"
            )
            print("[STATUS] Client: Waiting...")
    ...
    def start(self):
        ...
        if self.key_mapping:
            ...
            print(
                f"[CONFIG] Remapping Status: {'ENABLED' if self.remapping_enabled else 'DISABLED'}"
            )
            if self.enable_logging:
                for orig, mapped in self.key_mapping.items():
                    print(f"[CONFIG]   {orig} → {mapped}")
        ...
        while True:
            ...
            retry_count += 1
            print(
                f"[RETRY] Arduino connection failed (attempt {retry_count}). Retrying in 2s..."
            )
            time.sleep(2.0)
        ...
        if self.enable_hotkey_hook:
            try:
                if self.install_hook():
                    print("[HOOK] ✓ Keyboard hook installed (hotkeys only)")
                    print("[HOTKEYS] End = toggle key remapping")
                    print(
                        "[WARN] Hook is enabled - this causes delay even when passing through"
                    )
                    print(
                        "[WARN] Disable hook in config (enable_hotkey_hook=false) for ZERO delay"
                    )
                    ...
                    print("[WARN] Failed to install keyboard hook (hotkeys disabled)")
            except Exception as e:
                print(f"[WARN] Hook installation failed: {e}")
        else:
            print("[HOOK] ✓ Keyboard hook DISABLED (zero delay mode)")
            print(
                "[NOTE] End key hotkey disabled - remapping can be toggled via config restart"
            )
            print("[NOTE] Local input has ZERO delay - maximum performance")
    ...
    def stop(self):
        ...
        # Close Arduino (chỉ đóng nếu không phải DummySerial)
        if self.serial and self.is_serial_connected():
            if not (
                hasattr(self.serial, "__class__")
                and self.serial.__class__.__name__ == "DummySerial"
            ):
    ...
    def print_stats(self):
        """Print statistics"""
        print("\n=== STATISTICS ===")
        print(f"Arduino connected: {self.is_serial_connected()}")
