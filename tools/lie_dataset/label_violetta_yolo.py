"""GUI tool for labeling Violetta frames with bounding boxes (YOLO format)."""
...
class ViolettaYOLOLabeler:
    def __init__(self, root):
        self.root = root
        self.root.title("Violetta YOLO Labeler")
    ...
    def setup_ui(self):
        ...
        instructions_text = (
            "📋 Instructions:\n"
            "1. Click and drag to draw bounding box around Violetta with make-up (chỉ label Violetta ĐÚNG cần click)\n"
            "2. Skip animation frames (mờ/blur) - không label\n"
            "3. Sau shuffle: chỉ label Violetta ĐÚNG, không label 3 Violetta còn lại"
        )
        ...
        ttk.Button(nav_frame, text="Skip (No Violetta)", command=self.skip_frame).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def select_frames_dir(self):
        ...
        # If frames are in raw_frames/violetta_001/, labels go to labels/violetta_001/
    ...
    def skip_frame(self):
        """Skip frame (no Violetta in this frame)."""
    ...
    def save_session(self):
        ...
        session_file = os.path.join(
            project_root, "ai", "lie_detector", "violetta", "yolo_labeling_session.json"
        )
