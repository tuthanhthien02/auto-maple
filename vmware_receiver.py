...
def remote_toggle_bot():
    """
    Try to toggle the Auto Maple bot using the same logic as the GUI button.
...
class VMwareReceiver:
        ...
        try:
            if os.path.exists(self.CONFIG_PATH):
                with open(self.CONFIG_PATH, "r", encoding="utf-8") as f:
                    ...
                    self.obfuscation_enabled = config.get(
                        "obfuscation_enabled", self.obfuscation_enabled
                    )
                    if self.enable_logging:
                        print(
                            f"[CONFIG] Serial obfuscation: {'ENABLED' if self.obfuscation_enabled else 'DISABLED'}"
                        )
    ...
    def send_key_to_arduino(self, key_name: str, action: str) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                print(
                    f"[FORWARD] {action}:{key_name} ({total_written} bytes) → Arduino"
                )
            return True
        except Exception as e:
    ...
    def send_all_up(self):
        ...
        try:
            if self.serial and self.serial.is_open:
                ...
                if self.enable_logging:
                    print(
                        f"[FORWARD] all_up ({len(frames)} frame{'s' if len(frames) != 1 else ''})"
                    )
        except Exception as e:
    ...
    def start_server(self):
        ...
        try:
            ...
            print(
                f"[STATUS] Arduino: {'CONNECTED' if (self.serial and self.serial.is_open) else 'DISCONNECTED'}"
            )
