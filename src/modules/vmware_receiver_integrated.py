...
class VMwareReceiverIntegrated:
    ...
    def __init__(self, server_port=12345, enable_hotkey_hook=False, 
                 enable_logging=False, key_mapping=None):
        """
        ...
        """
        log.info("[VMwareReceiverIntegrated] Initializing VMware Receiver Integrated...")
        log.info(f"[VMwareReceiverIntegrated] Server port: {server_port}, Hotkey hook: {enable_hotkey_hook}")
        ...
        log.debug("[VMwareReceiverIntegrated] Getting SharedArduinoConnection instance...")
        self.shared_connection = SharedArduinoConnection.get_instance()
        arduino_connected = self.shared_connection.is_connected()
        log.info(f"[VMwareReceiverIntegrated] SharedArduinoConnection status: {'CONNECTED' if arduino_connected else 'DISCONNECTED'}")
        ...
        try:
            if os.path.exists(self.CONFIG_PATH):
                with open(self.CONFIG_PATH, 'r', encoding='utf-8') as f:
                    ...
                    self.enable_hotkey_hook = config.get('enable_hotkey_hook', self.enable_hotkey_hook)
                    ...
                    if key_mapping_config:
                        ...
                        if self.enable_logging:
                            log.info(f"[CONFIG] Loaded key remapping: {len(self.key_mapping)} mappings")
    ...
    def install_hook(self):
        ...
        try:
            ...
            if not self.hook:
                ...
                log.error(f"[ERROR] Failed to install keyboard hook. Error code: {error_code}")
                return False
    ...
    def handle_client(self, client_socket, client_address):
        ...
        try:
            ...
            log.info(f"[CLIENT] ✗ Disconnected from {client_address[0]}:{client_address[1]}")
            try:
                client_socket.close()
            except:
                pass
    ...
    def _server_loop(self):
        ...
        try:
            ...
            log.info(f"[SERVER] Listening for Host sender connection...")
            arduino_status = "CONNECTED" if self.shared_connection.is_connected() else "DISCONNECTED"
        ...
        if self.enable_hotkey_hook:
            try:
                if self.install_hook():
                    log.info(f"[HOOK] ✓ Keyboard hook installed (hotkeys only)")
                    log.info(f"[HOTKEYS] End = toggle key remapping")
                    # Start message loop thread for keyboard hook
                    self.hook_thread = threading.Thread(target=self.message_loop, daemon=True)
                    self.hook_thread.start()
                else:
                    log.warning(f"[WARN] Failed to install keyboard hook (hotkeys disabled)")
            except Exception as e:
                log.warning(f"[WARN] Hook installation failed: {e}")
        else:
            log.info(f"[HOOK] ✓ Keyboard hook DISABLED (zero delay mode)")
        
        # Start TCP server in background thread
        self.server_thread = threading.Thread(target=self._server_loop, daemon=True)
        self.server_thread.start()
        
        log.info(f"[VMwareReceiverIntegrated] ✅ TCP server starting in background thread (port {self.server_port})...")
    ...
    def stop_tcp_server(self):
        ...
        if self.server_socket:
            try:
                ...
                log.debug(f"[VMwareReceiverIntegrated] Error closing server socket: {e}")
            self.server_socket = None
