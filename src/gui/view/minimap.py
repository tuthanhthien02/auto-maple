...
class Minimap(LabelFrame):
    def __init__(self, parent, **kwargs):
        ...
        # Optimized canvas; default to 50% of previous width to reduce footprint
        self.WIDTH = 522  # Previously 1044; still keeps clarity at 1x scale
        self.HEIGHT = 328  # Maintain height for readability
    ...
    def display_minimap(self):
        ...
        if not minimap or minimap.get("minimap") is None:
            return
        ...
        minimap_hash = (
            hash(minimap["minimap"].tobytes())
            if minimap["minimap"] is not None
            else None
        )
        ...
        rune_tuple = (
            rune_active,
            rune_pos[0] if rune_pos else None,
            rune_pos[1] if rune_pos else None,
        )
        player_tuple = (
            player_pos[0] if player_pos else None,
            player_pos[1] if player_pos else None,
        )
