...
class VMwareReceiver(LabelFrame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # Status label
        self.status_label = tk.Label(self, text="Status: Not Running", fg="gray")
        ...
        if not hasattr(config, "_vmware_receiver_initialized"):
            ...
            if config.enable_vmware_receiver and not enabled:
                ...
                log.info(
                    "[VMWARE_RECEIVER_GUI] Ensuring VMware Receiver is disabled by default"
                )
    ...
    def _sync_to_config(self):
        ...
        try:
            ...
            log.debug(
                f"[VMWARE_RECEIVER_GUI] Synced to config: enabled={enabled}, port={port}, hotkey_hook={hotkey_hook}"
            )
    ...
    def _on_port_change(self, event=None):
        ...
        try:
            ...
            if not port_str:
                ...
                if port < 1 or port > 65535:
                    log.warning(
                        f"[VMWARE_RECEIVER_GUI] Invalid port: {port}, using default 12345"
                    )
                    port = 12345
                    self.port_var.set(str(port))
        except ValueError:
            log.warning(
                "[VMWARE_RECEIVER_GUI] Invalid port format, using default 12345"
            )
            port = 12345
            self.port_var.set(str(port))
        ...
        if config.vmware_receiver and config.vmware_receiver.is_running():
            log.info(
                f"[VMWARE_RECEIVER_GUI] Port changed to {port}, restarting server..."
            )
    ...
    def _on_hotkey_hook_change(self):
        ...
        if config.vmware_receiver and config.vmware_receiver.is_running():
            log.info(
                f"[VMWARE_RECEIVER_GUI] Hotkey hook changed to {hotkey_hook}, restarting server..."
            )
            config.vmware_receiver.stop_tcp_server()
    ...
    def _toggle_vmware_receiver(self, enabled: bool):
        ...
        try:
            if enabled:
                ...
                from src.modules.vmware_receiver_integrated import (
                ...
                log.info(
                    "[VMWARE_RECEIVER_GUI] ✅ VMware Receiver started successfully"
                )
            else:
    ...
    def _update_status(self):
        ...
        try:
            ...
            if (
                not hasattr(self, "status_label")
                or not self.status_label.winfo_exists()
            ):
                return
            ...
            if config.vmware_receiver and config.vmware_receiver.is_running():
                ...
                try:
                    arduino_connected = (
                        config.vmware_receiver.shared_connection.is_connected()
                    )
                    arduino_status = (
                        "Connected" if arduino_connected else "Disconnected"
                    )
                except Exception:
                ...
                self.status_label.config(text="Status: Not Running", fg="gray")
        except tk.TclError:
