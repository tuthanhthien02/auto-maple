...
class RoutineRandomization(LabelFrame):
    def __init__(self, parent, **kwargs):
        ...
        self._scroll_container.pack(side=tk.TOP, fill='both', expand=True, padx=0, pady=0)

        self._canvas = tk.Canvas(self._scroll_container, bd=0, highlightthickness=0, height=SCROLL_HEIGHT)
        self._canvas.pack(side=tk.LEFT, fill='both', expand=True)
        self._scrollbar = tk.Scrollbar(self._scroll_container, orient=tk.VERTICAL, command=self._canvas.yview)
        ...
        self.point_selection_frame = LabelFrame(self._content, 'Point Selection Randomization')
        ...
        self.routine_pattern_frame = LabelFrame(self._content, 'Routine Pattern Variation')
        ...
        self.command_sequence_frame = LabelFrame(self._content, 'Command Sequence Randomization')
        self.command_sequence_frame.pack(side=tk.TOP, fill='x', expand=True, padx=5, pady=5)

        command_enabled = self._load_bool_setting(COMMAND_SEQUENCE_KEY, 'routine_randomization.command_sequence.enabled')
        ...
        micro_enabled = self._load_bool_setting(
            MICRO_GESTURE_KEY,
            'routine_randomization.movement.micro_gesture.enabled'
        )
        ...
        self.dynamic_paths_frame.pack(side=tk.TOP, fill='x', expand=True, padx=5, pady=5)

        dynamic_paths_enabled = self._load_bool_setting(
            DYNAMIC_PATHS_KEY,
            'routine_randomization.dynamic_paths.enabled'
        )
        ...
        path_count = self._load_int_setting(DYNAMIC_PATHS_COUNT_KEY, 'routine_randomization.dynamic_paths.path_count', 4)
        ...
        self.skip_range_label = tk.Label(skip_row, text=f'{int(skip_min * 100)}%-{int(skip_max * 100)}%')
        ...
        self.switch_interval_label = tk.Label(switch_row, text=f'{switch_min}-{switch_max} loops')
    ...
    def _on_point_change(self):
        ...
        self._set_bot_config_bool('routine_randomization.point_selection.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_pattern_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.routine_pattern.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_backward_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.random_backward.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_command_sequence_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.command_sequence.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_position_offset_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.movement.position_offset.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_micro_gesture_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.movement.micro_gesture.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_dynamic_paths_change(self):
        ...
        self.settings.save_config()
        self._set_bot_config_bool('routine_randomization.dynamic_paths.enabled', enabled)
        self._sync_to_anti_detect()
        self._sync_to_routine()
    ...
    def _on_skip_range_change(self, value):
        ...
        self.skip_range_label.config(text=f'{int(skip_min * 100)}%-{int(skip_max * 100)}%')
    ...
    def _sync_to_anti_detect(self):
        ...
        ANTI_DETECT_CONFIG['routine_randomization']['movement']['position_offset']['enabled'] = \
            self.settings.get(POSITION_OFFSET_KEY)
        ANTI_DETECT_CONFIG['routine_randomization']['movement']['micro_gesture']['enabled'] = \
    ...
    def _sync_to_routine(self):
        ...
        try:
            ...
            if hasattr(config, 'routine') and config.routine is not None:
                ...
                config.routine.command_randomization['enabled'] = self.settings.get(COMMAND_SEQUENCE_KEY)
                # Update movement toggles
                config.routine.position_offset_config['enabled'] = self.settings.get(POSITION_OFFSET_KEY)
                config.routine.micro_gesture_config['enabled'] = self.settings.get(MICRO_GESTURE_KEY)
                
                # Update Dynamic Paths settings
                config.routine.dynamic_paths_enabled = self.settings.get(DYNAMIC_PATHS_KEY)
                if config.routine.dynamic_paths_enabled:
                    config.routine.dynamic_paths_config['path_count'] = self.settings.get(DYNAMIC_PATHS_COUNT_KEY)
                    config.routine.dynamic_paths_config['generation_strategy'] = self.settings.get(DYNAMIC_PATHS_STRATEGY_KEY)
                    ...
                    config.routine.dynamic_paths_config['selection_mode'] = self.settings.get(DYNAMIC_PATHS_SELECTION_MODE_KEY)
