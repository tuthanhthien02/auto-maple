...
class RoutineRandomizer:
    ...
    def should_skip_point(self, point_index, element):
        ...
        skip_prob = get_feature_value(
            "routine_randomization.point_selection.skip_probability", 0.10
        )
        max_skip = get_feature_value(
            "routine_randomization.point_selection.max_skip_per_loop", 2
        )
        min_between = get_feature_value(
            "routine_randomization.point_selection.min_points_between_skips", 3
        )
        never_skip_labels = get_feature_value(
            "routine_randomization.point_selection.never_skip_labels", True
        )
        never_skip_jumps = get_feature_value(
            "routine_randomization.point_selection.never_skip_jumps", True
        )
        never_skip_transitions = get_feature_value(
            "routine_randomization.point_selection.never_skip_transitions", True
        )
    ...
    def should_switch_variant(self):
        ...
        min_loops = get_feature_value(
            "routine_randomization.routine_pattern.min_loops_before_switch", 3
        )
        switch_prob = get_feature_value(
            "routine_randomization.routine_pattern.variant_switch_probability", 0.15
        )
    ...
    def select_new_variant(self):
        ...
        variants_config = get_feature_value(
            "routine_randomization.routine_pattern.variants", {}
        )
    ...
    def get_variant_start_index(self):
        ...
        # Fallback: return 0
        log.warning(
            "Could not find start label '%s' for variant, using default",
            start_label_name,
        )
        return 0
...
def initialize_routine_randomization():
    ...
    try:
        ...
        log.debug(
            "Could not load routine randomization settings (will use defaults): %s", e
        )
