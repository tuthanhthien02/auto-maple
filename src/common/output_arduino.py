...
class ArduinoSerialOutput:
    ...
    def __init__(
        ...
        log.info(
            f"[ArduinoSerialOutput] Connection status: {'CONNECTED' if self.connected else 'DISCONNECTED'}"
        )
        if self.connected:
            log.info(
                f"[ArduinoSerialOutput] COM Port: {self.com_port}, Baudrate: {self.baudrate}"
            )
    ...
    def send_command(self, action: str, key: str = None) -> bool:
        ...
        if old_connected != self.connected:
            log.info(
                f"[ArduinoSerialOutput] Connection status changed: {'CONNECTED' if self.connected else 'DISCONNECTED'}"
            )

        if not self.connected:
            log.debug(
                f"[ArduinoSerialOutput] Not connected, cannot send command: {action}:{key if key else 'all_up'}"
            )
            return False
        ...
        if not result:
            log.debug(
                f"[ArduinoSerialOutput] Failed to send command: {action}:{key if key else 'all_up'}"
            )
        return result

    def press(
        self, key: str, n: int = 1, down_time: float = 0.05, up_time: float = 0.1
    ):
        """
    ...
    def press_with_behavioral_pause(
        self, key: str, n: int = 1, down_time: float = 0.05, up_time: float = 0.1
    ):
        """
    ...
    def press_sequence_with_variation(self, keys: list, delays: list = None):
        ...
        if delays is None:
            if use_timing_randomization:
                delays = [
                    _get_human_like_delay(0.1, "micro") for _ in range(len(keys) - 1)
                ]
            else:
            ...
            if i < len(keys) - 1:
                delay = (
                    ...
                    if i < len(delays)
                        ...
                        0.1
                        if not use_timing_randomization
                        else _get_human_like_delay(0.1, "micro")
                    )
                )
                time.sleep(delay)
