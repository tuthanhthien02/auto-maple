...
class MetricsLogger:
    ...
    def calculate_randomization_effectiveness(self) -> float:
        ...
        with self.lock:
            ...
            backward_rate = self.randomization_stats['backward_movements'] / max(total_actions, 1)
            shuffle_rate = self.randomization_stats['command_shuffles'] / max(total_actions, 1)
    ...
    def get_summary(self) -> Dict:
        ...
        with self.lock:
            ...
            avg_loop_duration = (
                ...
                if self.loop_durations else 0.0
            )
    ...
    def log_summary(self):
        ...
        log.info("❌ Errors: %d (rate: %.2f%%)", 
                summary['error_count'], summary['error_rate'] * 100)
        log.info("🔧 Recoveries: %d", summary['recovery_count'])
        log.info("")
        log.info("🎲 RANDOMIZATION STATS:")
        log.info("   Point skips: %d (%.1f%%)", 
                summary['randomization_stats']['point_skips'],
                summary['skip_rate'] * 100)
        log.info("   Point executions: %d", 
                summary['randomization_stats']['point_executions'])
        log.info("   Backward movements: %d", 
                summary['randomization_stats']['backward_movements'])
        log.info("   Command shuffles: %d", 
                summary['randomization_stats']['command_shuffles'])
        log.info("   Command skips: %d", 
                summary['randomization_stats']['command_skips'])
        log.info("   Extra waits: %d", 
                summary['randomization_stats']['extra_waits'])
        log.info("   Position offsets: %d", 
                summary['randomization_stats']['position_offsets'])
        log.info("   Micro gestures: %d", 
                summary['randomization_stats']['micro_gestures'])
        log.info("   Effectiveness score: %.2f/1.0", 
                summary['randomization_effectiveness'])
        log.info("")
