...
def check_arduino_connection():
    ...
    print(
        f"   config.arduino_com_port = {getattr(config, 'arduino_com_port', 'NOT SET')}"
    )
    print(
        f"   config.arduino_baudrate = {getattr(config, 'arduino_baudrate', 'NOT SET')}"
    )
            ...
            if hasattr(arduino_output, "connected"):
                ...
                if arduino_output.connected:
                    ...
                    if hasattr(arduino_output, "shared_connection"):
                        print("   Using SharedArduinoConnection: ✅")
                        shared_conn_check = arduino_output.shared_connection
                        if shared_conn_check is shared_conn:
                            print("   Same instance as SharedArduinoConnection: ✅")
                        else:
                            print("   ⚠️  Different instance!")
                    else:
                        print("   ⚠️  NOT using SharedArduinoConnection!")
                else:
                    print("   ⚠️  Arduino output NOT connected!")
            else:
                print(f"   Arduino output instance: {arduino_output}")
        else:
            print("   Arduino output: False (not initialized or disabled)")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    print(
        f"{'✅' if shared_connected else '❌'} SharedArduinoConnection: {shared_connected}"
    )
    print(
        f"{'✅' if vkeys_connected else '❌'} VKeys Arduino Output: {vkeys_connected}"
    )
